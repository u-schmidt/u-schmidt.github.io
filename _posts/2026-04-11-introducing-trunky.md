...
So that's why I started this side project: to become more familiar with all that and learn how to build modern, AI-oriented applications. And I would like to share my journey with you.
...
- show how an AI backend architecture can be built for tool use
...
- a case study on how even small, locally hosted AI models can be used to build apps that would not have been possible a few years ago

If you're interested in how to design an intelligent desktop assistant, Trunky might be a useful case study.

## What Trunky Is

The initial idea behind Trunky was to build a simple chat application with a locally hosted AI model that could then be used to experiment with more advanced features. The goal has become to extend Trunky into an automatic knowledge-management system that gathers, organizes, and retrieves information from chat conversations in a way that feels *agentic*: autonomous, planned, purposeful, and somehow intelligent.
...
In the image above, you can see Trunky's main screen. Like many other AI chat applications, you have your chat history and input area in the middle, along with side panes for managing conversations and workspace information.

## The Overall Function

Trunky's UI is divided into four pages, which you can navigate through the left navigation bar:

**Chat Page**
The main view of the application. Here you can chat with the LLM model and use it to store and access knowledge items. It works like a mini-RAG system: RAG (**R**etrieval **A**ugmented **G**eneration) is a framework that allows an AI model to access and process data that were not part of its training, such as a company's database, without having to put them into the (often limited) context window.

Trunky can automatically filter information from your conversation and store it in the workspace. The software then uses this data as an additional source of information, like in the example below.
...
This file is where the assistant gets its “mental model” of the workspace and its operational rules. It is easy to underestimate the impact of your instructions and exact formulations in your prompts, so this part may seem less important than it actually is. I put it in second place to emphasize its relevance.
...
- `updateInformation` to update existing or insert new information
- `forgetInformation` to invalidate a specific information (soft delete)
- `restoreInformation` to restore information that has been (soft) deleted before
...
- instructs it to prefer tools when a dataset or memory is needed
